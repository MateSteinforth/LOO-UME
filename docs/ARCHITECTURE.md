...
| `src/cad/GenerateStructuralSolids.ts` | Loft broad cap-derived surfaces between nearest eligible screw shoes and Boolean-union co-located paths into one local multi-panel junction with Manifold | A short 6 mm cubic rear-normal departure keeps the ribbon near panel level; a 0.001 mm Manifold simplification removes numerical Boolean slivers before exact anchor, print-envelope, fastener, clearance, topology, triangle, and volume checks gate each final mesh |
