...
structuralDesign -> panel pairs + local junction groups -> selected Manifold connector surfaces
...
fingerprint. Printable Manifold bracket/hub/strut solids are also implemented.
Mesh export and publication remain the ordered `TRUSS-016` through `TRUSS-018`
tasks. This branch does not replace the two existing fabrication routes.
...
| `src/cad/GenerateStructuralSolids.ts` | Generate the selected screw-shoe ribbon or a 2 mm ruled full-edge bridge between 5 mm ridges at the profile LED planes, then Boolean-union co-located paths into one local multi-panel junction with Manifold | Panel poses and the profile remain authoritative; screw-only voids, PCB clearance, one-component topology, print-envelope, triangle, and volume checks gate each final mesh |
...
| `src/sculpture/SculptureEditor.ts` | Add/surface-place/free-XYZ-transform/delete/seed and mechanics invalidation | Free transforms normalize the authoritative right-handed pose and remove stale surface attachment; editing does not require successful CAD |
...
Structural CAD emits one loft body per panel-pair cell. Broad 13 mm rounded
