...
does not yet prove an ESP32 installation. The 41-panel project stores only the
draft chain lengths `11/10/10/10`; the route is regenerated by a geographic
heuristic, GPIOs are unknown, installed rotations/mirroring are unknown, panel
pixel order is provisional, and RGB/GRB color order is not part of the contract.
The parser also requires provisional wiring while readiness requires measured
wiring, so the current schema cannot represent a completed production state.
