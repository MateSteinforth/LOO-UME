...
Selected panels show translation arrows and rotation rings at the same time.
Both controls use the panel's local X, Y, and Z axes. A free transform writes
one normalized, right-handed saved pose and removes `surfaceAttachment`,
because the old triangle and barycentric
