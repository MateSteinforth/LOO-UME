...
describe("LOO/UME managed launcher", () => {
  it("serializes concurrent starts, reopens one server, reports status, and stops it", async () => {
    ...
    expect(
      (await readFile(fixture.actions, "utf8")).trim().split("\n"),
    ).toEqual(["launch"]);
    expect(
    ...
    expect(
      (await readFile(fixture.opened, "utf8")).trim().split("\n"),
    ).toHaveLength(3);
  ...
  it("does not steal an atomically published live launch lock", async () => {
    ...
    await expect(
      ...
      stderr: expect.stringContaining(
        "another launch did not finish within 1 seconds",
      ),
    });
    expect(await readlink(lockPath)).toBe(String(first.pid));
  ...
  it("honors a verified live legacy owner lock", async () => {
    ...
    try {
      ...
      expect(
        await readFile(join(fixture.state, "launch.lock", "owner.pid"), "utf8"),
      ).toBe(`${holder.pid}\n`);
    } finally {
      holder.kill("SIGTERM");
  ...
  it("rejects an invalid port before starting anything", async () => {
    ...
    await expect(stat(fixture.actions)).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
  ...
  it("rejects an unrelated HTTP service on the selected port", async () => {
    ...
    try {
      ...
      await expect(
        execFileAsync("sh", [fixture.launcher, "--stop"], {
          env: { ...fixture.environment, LOO_UME_PORT: String(port) },
          timeout: 5_000,
        }),
      ).rejects.toMatchObject({
  ...
  it("retains the legacy self-installing application and tagged release contract", async () => {
    ...
    const uninstaller = await readFile(
      "macos/Uninstall LOO UME.command",
      "utf8",
    );
    ...
    const recoveryIndex = application.indexOf(
      'recover_orphaned_server_record\n  progress "Stopping',
    );
    const stopIndex = application.indexOf(
      '"$checkout_root/scripts/looume.sh" --stop',
    );
    expect(recoveryIndex).toBeGreaterThanOrEqual(0);
    expect(stopIndex).toBeGreaterThanOrEqual(0);
    expect(recoveryIndex).toBeLessThan(stopIndex);
    ...
    expect(workflow).toContain(
      "release_tag=mac-launcher-review-$GITHUB_RUN_NUMBER",
    );
    ...
    expect(workflow).toContain("permissions:\n      contents: write");
    const packagedLauncherIndex = uninstaller.indexOf(
      'if [ -x "$packaged_launcher" ]',
    );
    const installedLauncherIndex = uninstaller.indexOf(
      'if [ -x "$installed_launcher" ]',
    );
    expect(packagedLauncherIndex).toBeGreaterThanOrEqual(0);
    expect(installedLauncherIndex).toBeGreaterThanOrEqual(0);
    expect(packagedLauncherIndex).toBeLessThan(installedLauncherIndex);
  ...
  it("recovers and stops an orphaned managed server after its PID record was lost", async () => {
    ...
    for (let attempt = 0; attempt < 100; attempt += 1) {
      try {
        const response = await fetch(
          `http://127.0.0.1:${port}/api/generator-status`,
        );
    ...
    try {
      ...
      expect(await readFile(join(fixture.state, "server.pid"), "utf8")).toBe(
        `${server.pid}\n`,
      );
      await expect(stat(fixture.actions)).rejects.toMatchObject({
        code: "ENOENT",
  ...
  it("restarts an owned API-only server whose editor files are gone", async () => {
    ...
    try {
      ...
      expect(launched.stdout).toContain(
        "owned server without its editor files",
      );
      expect((await readFile(fixture.actions, "utf8")).trim()).toBe("launch");
  ...
  it("repairs an orphan record in the app before delegating to an older checkout", async () => {
    ...
    try {
      ...
      expect(result.stdout).toContain(
        "Recovered the existing managed LOO/UME server",
      );
  ...
  it("persists the next free port when another checkout owns the default", async () => {
    ...
    const foreignServer = createHttpServer((_request, response) => {
      response.setHeader("Content-Type", "application/json");
      response.end(
        JSON.stringify({ schemaVersion: "1.0.0", generator: "manifold" }),
      );
    });
    await new Promise<void>((resolvePromise, reject) => {
    ...
    try {
      ...
      expect(
        await readFile(
          join(checkout, ".tools", "looume", "server.port"),
          "utf8",
        ),
      ).toBe(`${port + 1}\n`);
    } finally {
      await new Promise<void>((resolvePromise) =>
        foreignServer.close(() => resolvePromise()),
      );
    }
  });
  ...
  it("hands Terminal the stable installed application path", async () => {
    ...
    await writeFile(
      fakeDitto,
      ["#!/bin/sh", 'touch "$FAKE_DITTO_LOG"', "exit 1", ""].join("\n"),
    );
    ...
    expect(invocation).not.toContain(
      resolve("macos/launcher/Contents/MacOS/LOO-UME"),
    );
  ...
  it("backs up local projects and removes only the managed Mac installation", async () => {
    ...
    expect(
      await readFile(
        join(home, "Documents", backups[0]!, "saved.loo.zip"),
        "utf8",
      ),
    ).toBe("project bytes");
  });
  ...
  it("installs a verified checkout atomically and creates the Mac command", async () => {
    ...
    expect(
      (await stat(join(support, "application", ".git"))).isDirectory(),
    ).toBe(true);
    ...
    expect(
      (await readdir(support)).some((name) =>
        name.startsWith("install.lock.claim."),
      ),
    ).toBe(false);
    expect(
      (await readdir(support)).some((name) =>
        name.startsWith(".application.stage"),
      ),
    ).toBe(false);
  });
  ...
  it("updates an existing managed installation after replacing its app", async () => {
    ...
    await writeFile(
      join(checkout, "scripts", "looume.sh"),
      ["#!/bin/sh", 'printf \'%s\\n\' "$1" >> "$FAKE_ACTION_LOG"', ""].join(
        "\n",
      ),
    );
    ...
    await expect(
      stat(join(support, ".pending-installer-upgrade")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    expect(
      await readFile(join(installedApp, "Contents", "Info.plist"), "utf8"),
  ...
  it("recovers an interrupted application copy before replacing the launcher", async () => {
    ...
    await writeFile(
      join(applications, ".LOO-UME.app.stage", "partial"),
      "partial",
    );
    const fakeDitto = join(home, "ditto.sh");
    await writeFile(
    ...
    expect(
      await readFile(join(installed, "Contents", "Info.plist"), "utf8"),
    ).toContain("art.loo-ume.launcher");
    expect(await readFile(openLog, "utf8")).toBe(`${installed}\n`);
    expect(
      (await readdir(applications)).some((name) =>
  ...
  it("removes a failed first-install staging checkout", async () => {
    ...
    await chmod(fakeGit, 0o755);
    await expect(
      execFileAsync("sh", ["macos/launcher/Contents/MacOS/LOO-UME"], {
        env: {
          ...process.env,
          HOME: home,
          LOO_UME_SUPPORT_ROOT: support,
