...
const firmwareReceipt = JSON.parse(
  readFileSync("firmware/build-receipt.json", "utf8"),
) as {
  artifact: { name: string; byteLength: number; sha256: string };
  fullFlashArtifact: { name: string; byteLength: number; sha256: string };
};
...
describe("guarded WLED deployment contract", () => {
  it("emits the exact pinned installation files and validates their identity", async () => {
    ...
    expect(bundle.deploymentIdentity).toBe(
      sha256ExactBytes(bundle.manifestBytes),
    );
    expect(JSON.parse(bundle.manifestBytes)).toMatchObject({
      ...
      firmware: {
        ...
        artifact: {
          ...
          byteLength: firmwareReceipt.artifact.byteLength,
          sha256: firmwareReceipt.artifact.sha256,
        },
        fullFlashArtifact: {
          name: "wled-orbital-esp32dev-full-flash.bin",
          byteLength: firmwareReceipt.fullFlashArtifact.byteLength,
          sha256: firmwareReceipt.fullFlashArtifact.sha256,
          flashAddress: 0,
          eraseAll: true,
        },
      },
    ...
    expect(
      JSON.parse(bundle.files.get("wled/one-panel-smoke-cfg.json")!),
    ).toMatchObject({
  ...
  it("uses diagnostic-only names for draft or explicitly diagnostic output", async () => {
    ...
    expect(
      [...bundle.files.keys()].every((path) => path.includes("diagnostic")),
    ).toBe(true);
    expect(() =>
      createWledDeploymentBundle(draft, sculptureBytes, "installation"),
    ...
    expect(() =>
      createWledDeploymentBundle(stale, sculptureBytes, "installation"),
    ).toThrow(/saved route/);
    expect(() =>
      validateWledDeploymentBundle(
        bundle.manifestBytes,
  ...
  it("rejects stale routes, source bytes, artifacts, identities, and bus data", async () => {
    ...
    expect(() =>
      validateWledDeploymentBundle(bundle.manifestBytes, files, "0".repeat(64)),
    ...
    const configEntry = changedManifest.files.find(
      ({ path }) => path === "wled/cfg.json",
    )!;
    configEntry.byteLength = new TextEncoder().encode(
      changedConfigBytes,
    ).byteLength;
    configEntry.sha256 = sha256ExactBytes(changedConfigBytes);
    const changedManifestBytes =
