...
test("persists the manual wiring rotation gate from Developer utilities", async ({
  ...
  await expect(page.locator("#viewer")).toHaveAttribute(
    "data-panel-mounting-hole-count",
    "12",
  );
  await expect(page.locator("#viewer")).toHaveAttribute(
    "data-panel-mounting-hole-faces",
    "back-only",
  );
  await expect(page.locator("#viewer")).toHaveAttribute(
    "data-panel-din-hole-color",
    "4ade80",
  );
  await expect(page.locator("#viewer")).toHaveAttribute(
    "data-panel-dout-hole-color",
    "ff4d6d",
  );
