...
describe("guarded ESP32 setup contracts", () => {
  ...
  it("enables automatic reconnect only after setup or prior CP2102 permission", async () => {
    ...
    await expect(
      automaticEsp32ReconnectAvailable(storage, noPorts),
    ).resolves.toBe(false);
    await expect(
      automaticEsp32ReconnectAvailable(undefined, noPorts),
    ).resolves.toBe(false);
    ...
    await expect(
      automaticEsp32ReconnectAvailable(storage, {
        getPorts: async () => [approvedPort],
      }),
    ).resolves.toBe(true);
    await expect(
      automaticEsp32ReconnectAvailable(undefined, noPorts, async () => true),
    ).resolves.toBe(true);
  ...
  it("reads and records desktop reconnect authorization", async () => {
    ...
    await expect(desktopEsp32ReconnectAvailable(statusRequest)).resolves.toBe(
      true,
    );
    ...
    await expect(rememberDesktopEsp32Reconnect(enableRequest)).resolves.toBe(
      true,
    );
  ...
  it("accepts only the measured CP2102 and classic ESP32/WLED identity", () => {
    ...
    expect(() =>
      assertApprovedEsp32Chip("ESP32-D0WDQ6 (revision 1)"),
    ).not.toThrow();
    expect(() => assertApprovedEsp32Chip("ESP32-S3")).toThrow(
      /approved classic ESP32/,
    );
    expect(() =>
      assertApprovedImprovIdentity({
        firmware: "WLED",
  ...
  it("derives bounded buses through the complete loaded 41-panel simulator", () => {
    ...
    const replacementConfig = createSimulatorSetupConfig(
      template,
      [
        { startIndex: 0, pixelCount: 704, gpio: 21 },
        { startIndex: 704, pixelCount: 640, gpio: 22 },
        { startIndex: 1_344, pixelCount: 640, gpio: 25 },
        { startIndex: 1_984, pixelCount: 640, gpio: 26 },
      ],
      0,
    ) as { hw: { led: { ins: Array<{ pin: number[] }> } } };
    expect(replacementConfig.hw.led.ins.map((output) => output.pin)).toEqual([
      [21],
      [22],
      [25],
      [26],
    ]);
    const nonLegacy2624 = createSimulatorSetupConfig(
      template,
      [
      ...
      0,
      82,
    ) as { hw: { led: { maxpwr: number; ins: Array<{ maxpwr: number }> } } };
  ...
  it("requires the exact generated ledmap JSON", () => {
    expect(() =>
      assertLedmapReadback('{"map":[0,2,1]}\n', '{"map":[0,2,1]}\n'),
    ).not.toThrow();
    expect(() =>
      assertLedmapReadback('{"map":[0,1,2]}', '{"map":[0,2,1]}'),
    ).toThrow(/does not match/);
    expect(() => assertLedmapReadback("not-json", '{"map":[0]}')).toThrow(
      /invalid JSON/,
    );
  });
  ...
  it("rejects an async ESP32 result after the loaded project changes", async () => {
    ...
    const completion = oldRequest.then((deviceUrl) => {
      if (
        isCurrentSimulatorSetup(oldPayload, currentRevision, currentFingerprint)
      )
  ...
  it("cancels automatic discovery before another stale-project attempt", async () => {
    ...
    const delay = vi.fn();
    await expect(
      retryExistingSimulatorDiscovery(discover, 12, delay, () => current),
  ...
  it("reports the exact Wi-Fi provisioning stage", async () => {
    ...
    ).rejects.toThrow(/did not connect to AZ24 within 60 seconds/);
  });
  ...
  it("accepts only local private device addresses", () => {
    ...
    expect(() => privateDeviceUrl("https://192.168.68.51/")).toThrow(
      /non-HTTP/,
    );
    expect(() => privateDeviceUrl("http://8.8.8.8/")).toThrow(
      /private network/,
    );
  ...
  it("binds the mDNS name and current DHCP address to the same device MAC", async () => {
    ...
    await expect(
      resolveVerifiedWledAddress("aa:bb:cc:dd:ee:ff"),
    ).resolves.toEqual(new URL("http://192.168.68.72/"));
    ...
    await expect(
      resolveVerifiedWledAddress("aa:bb:cc:dd:ee:ff"),
    ).rejects.toThrow(/different WLED device/);
  ...
  it("maps simulator names to live WLED IDs and fails when a name is absent", () => {
    ...
    expect(() =>
      remapStateToLiveTables(payload(), ["Solid"], ["Default", "Forest"]),
    ).toThrow(/does not match/);
  });
  ...
  it("requires exact live config and state read-back", () => {
    ...
    expect(() =>
      assertConfigReadback({ ...config, id: { mdns: "wled" } }, value),
    ...
    expect(() => assertStateReadback({ ...state, bri: 127 }, value)).toThrow(
      /state read-back/,
    );
    ...
    expect(() =>
      assertStandaloneStateReadback(standaloneState, value),
    ).not.toThrow();
    expect(() =>
      assertStandaloneStateReadback({ ...standaloneState, ps: 0 }, value),
  ...
  it("sends a mapped physical framebuffer through the fixed device broker", async () => {
    const pixels = Array.from(
      { length: 64 },
      (_, index) => [index, 255 - index, index % 3] as [number, number, number],
    );
    ...
    const expected = pixels.flatMap((pixel) =>
      pixel.map((channel) => Math.floor((channel / 255) ** 2.2 * 255 + 0.5)),
    );
  ...
  it("gamma-corrects dim DDP colors like native WLED rendering", async () => {
    ...
    vi.stubGlobal("fetch", fetchMock);
    await sendSimulatorFramebuffer(new URL("http://192.168.68.53/"), [
      [5, 8, 22],
      [255, 122, 24],
    ]);
  ...
  it("writes and verifies the native standalone boot preset", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(
        new Response(JSON.stringify(["Solid", "Blink", "Rainbow"])),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify(["Default", "Forest"])),
      )
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ leds: { bootps: 1 } })),
      );
    vi.stubGlobal("fetch", fetchMock);
    ...
    const saveBody = JSON.parse(String(saveRequest[1].body)) as Record<
      string,
      unknown
    >;
  ...
  it("reconciles a lost state-write response without repeating the mutation", async () => {
    ...
    expect(updates).toEqual([
      expect.stringMatching(
        /state-write response was lost.*Verifying the exact saved state/,
      ),
    ]);
  });
  ...
  it("orders simulator pixels by the first panel's physical addresses", () => {
    ...
    expect(() => mappedPanelFramebuffer(pixels, entries.slice(1), 0)).toThrow(
      /contiguous mapped output/,
    );
  });
  ...
  it("reconnects only to the exact persisted loaded simulator device and ledmap", async () => {
    ...
    await expect(
      connectExistingSimulatorDevice(value, {
    ...
    fetchMock.mockReset().mockResolvedValueOnce(new Response("not-json"));
    await expect(
      synchronizeDeviceLedmap(
        new URL("http://192.168.68.53/"),
        value.ledmapBytes,
    ...
    const interruptedContinuation = vi
      .fn()
  ...
  it("retries the complete restarted snapshot and keeps exact ledmap checks", async () => {
    ...
    const ledmapReads = paths
      .map((path, index) =>
        path === "/edit?func=edit&path=/ledmap.json" ? index : -1,
      )
  ...
  it("stops restarted snapshot retries at the strict wall-clock deadline", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockRejectedValue(new Error("device still restarting"));
    vi.stubGlobal("fetch", fetchMock);
    const value = payload();
