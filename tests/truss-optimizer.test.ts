...
async function normalizedProject(
  path: string,
): Promise<NormalizedStructuralDesign> {
...
describe("structural truss optimization", () => {
  it("removes consistently low-load candidates and preserves redundant connectivity", async () => {
    ...
    expect(optimized.optimizedCandidate.members.length).toBeLessThan(
      candidate.members.length,
    );
    expect(optimized.trace.some(({ action }) => action === "remove")).toBe(
      true,
    );
    expect(optimized.trace.at(-1)?.action).toBe("converged");
    expect(optimized.trace.length).toBeLessThanOrEqual(
      optimized.policy.maximumIterations,
    );
    expect(() =>
      validateCandidateTruss(optimized.optimizedCandidate),
    ).not.toThrow();
    expect(
      optimized.optimizedCandidate.connectorCells.every((cell) =>
        cell.sideNodeIds.flat().every((nodeId) =>
          cell.memberIds.some((memberId) => {
            const member = optimized.optimizedCandidate.members.find(
              ({ id }) => id === memberId,
            )!;
  ...
  it("rounds resized members up to printable increments and updates self-weight", async () => {
    const normalized = await loadedTwoPanel(-50);
    normalized.design.fabrication.maximumMemberDiameterMm = 14.1;
    const candidate = createCandidateTruss(normalized);
    const initialMass = candidate.members.reduce(
      (sum, member) =>
        sum +
        (member.analysisOnly
          ? 0
          : ((Math.PI * member.initialDiameterMm ** 2) / 4) *
            member.lengthMm *
            1e-9 *
    ...
    expect(optimized.status).toBe("converged");
    expect(optimized.trace.some(({ action }) => action === "resize")).toBe(
      true,
    );
    const printableMemberIds = new Set(
      optimized.optimizedCandidate.members
        .filter(({ analysisOnly }) => !analysisOnly)
        .map(({ id }) => id),
    );
    expect(
      optimized.members
        .filter(({ memberId }) => printableMemberIds.has(memberId))
        .every(
          ({ diameterMm }) =>
            diameterMm >=
              normalized.design.fabrication.minimumMemberDiameterMm &&
            diameterMm <=
              normalized.design.fabrication.maximumMemberDiameterMm &&
            Number.isInteger(
              (diameterMm -
                normalized.design.fabrication.minimumMemberDiameterMm) /
    ...
    expect(
      optimized.analysis.loadCases[0]!.maximumDisplacementMm,
  ...
  it("reports explicit infeasible and iteration-limit outcomes", async () => {
    ...
    expect(infeasible.violations).toEqual(
    ...
    const boundedInput = await loadedTwoPanel(-50);
  ...
  it("rejects an unloaded-force ratio above one", async () => {
    ...
    expect(() =>
      optimizeStructuralTruss(normalized, candidate, {
  ...
  it("is deterministic when candidate arrays are reordered", async () => {
    ...
    expect(optimizeStructuralTruss(normalized, reordered)).toEqual(
      optimizeStructuralTruss(normalized, candidate),
    );
  });
});
