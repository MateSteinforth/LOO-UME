...
export function createDdpPackets(
  ...
  if (
    ...
    throw new Error(
      "The DDP preview requires from 1 through 2,624 RGB pixels.",
    );
  }
  ...
  for (
    ...
    offset += DDP_CHANNELS_PER_PACKET
  ) {
...
export function createDdpFrameSender(
  ...
  return {
    send(address, packets) {
      if (closed)
        return Promise.reject(new Error("The WLED DDP sender is closed."));
      if (sending)
        return Promise.reject(
          new Error("A WLED DDP frame is already sending."),
        );
      if (packets.length === 0) {
        return Promise.reject(
          new Error("A DDP frame requires at least one packet."),
        );
      }
...
const resolveIpv4: ResolveIpv4 = async (hostname) =>
  (await lookup(hostname, { all: true, family: 4 })).map(
    (entry) => entry.address,
  );
...
export async function resolvedEsp32Target(
  ...
  if (
    addresses.length === 0 ||
    addresses.some((candidate) => !privateIpv4(candidate))
  ) {
    throw new Error(
      "The WLED mDNS name did not resolve only to private addresses.",
    );
  }
...
async function requestBody(
  ...
  for await (const chunk of request) {
    ...
    if (size > MAX_BODY_BYTES)
      throw new Error("ESP32 request body is too large.");
    chunks.push(bytes);
  }
    ...
    async handle(request, response) {
      ...
      if (!isLoopbackHost(request.headers.host)) {
        sendJson(response, 403, {
          error: "ESP32 access accepts only a loopback Host.",
        });
        return true;
      }
      if (request.headers["x-loo-ume-esp32"] !== "1") {
        sendJson(response, 403, {
          error: "ESP32 access requires the editor authorization header.",
        });
        return true;
      }
      if (requestUrl.pathname === "/api/esp32-frame") {
        ...
        if (
          ...
          sendJson(response, 400, {
            error: "The ESP32 realtime request is not allowed.",
          });
          return true;
        }
        try {
      ...
      try {
        ...
        sendJson(response, 400, {
          error: "The ESP32 target or request is not allowed.",
        });
        return true;
      }
      try {
