...
function localProjectSource(filename: string): string {
  ...
  return `./api/project-library/package/local/${encodeURIComponent(filename)}`;
}
...
export async function saveLocalProjectPackage(
  ...
  return mutationResponse(await fetch(localProjectSource(filename), {
    method: "PUT",
    headers: {
...
export async function deleteLocalProjectPackage(
  ...
  const response = await fetch(localProjectSource(filename), {
    method: "DELETE",
