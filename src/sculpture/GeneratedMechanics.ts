...
export function assertPortableProjectAssetSource(
  ...
  if (typeof source !== "string" || source.length === 0) {
    throw new Error(`${label} source must be a non-empty project-relative path.`);
  }
  ...
  if (
    ...
    throw new Error(
      `${label} source must be a safe portable path relative to sculpture.json.`,
    );
  }
}

export function assertProjectAssetReference(
  reference: unknown,
  label: string,
): asserts reference is ProjectAssetReference {
  if (typeof reference !== "object" || reference === null || Array.isArray(reference)) {
    throw new Error(`${label} must be an asset reference.`);
  }
...
export function createGeneratedMechanicsFingerprint(
  ...
  const input = {
    ...
    panelProfile: {
      ...
      mounting: {
        ...
        pcbHolePreviewDiameter: profile.mounting.pcbHolePreviewDiameter,
        ...
        holes: [...profile.mounting.holes]
          .sort((left, right) => compareText(left.id, right.id)),
        capAllocation: profile.mounting.capAllocation,
        physicalCorrections: {
...
export function getGeneratedMechanicsState(
  definition: PanelAssemblyDefinition,
  profile: PanelHardwareProfile,
): GeneratedMechanicsState {
  const manifest = definition.generatedMechanics;
  if (!manifest) return "absent";
  return manifest.sourceFingerprint.value ===
