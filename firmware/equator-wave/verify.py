...
for name, expected in baseline["inputs"].items():
    ...
    if name == "wled00/wled.h":
        assert content.count(b"#define VERSION 2609102") == 1
        content = content.replace(b"#define VERSION 2609102", b"#define VERSION 2609099")
...
assert runtime["source"]["glitchAudioHeader"]["sha256"] == sha(HERE / "GlitchAudio.h")
...
for name in ("mode_equator_wave", "mode_packet_fault", "mode_bit_rain", "mode_spectral_gates", "EquatorWaveUsermod", "FFTcode", "looDdpReset", "loo_rmt_encode_led_strip"):
...
artifact = output / "wled-equator-wave-2609102.bin"
...
receipt = {
    ...
    "effectName": "Equator Wave, Packet Fault, Bit Rain, Spectral Gates",
