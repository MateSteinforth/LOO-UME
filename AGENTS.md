...
- Codex, Grok, and other agents may work concurrently. Never discard unfamiliar
  changes or assume exclusive ownership of a branch, worktree, file, or remote
  ref.
...
- Parallelize only independent seams. Do not assign concurrent edits to the
  same file or subsystem unless one orchestrator coordinates the overlap in
  `TASKS.md`.
...
- Route work by required depth, not by agent brand:
  - **Grok — full reasoning:** orchestration, architecture, difficult
    debugging, ambiguous or high-risk changes, major refactors, complex
    math/geometry/3D, and final review when correctness matters.
  - **Grok — lighter path:** ordinary feature implementation, medium-complexity
    debugging, bounded refactors, tests, repository exploration, call-site
    location, summarization, test/build/log inspection, mechanical edits,
    documentation, simple fixes, and independent first-pass review.
  - **Codex (when present):** GPT-5.6 Luna for bounded inspection, tests, logs,
    mechanical edits, and first-pass review; Terra for normal implementation
    and medium debugging; Sol for orchestration, architecture, hard debugging,
    high-risk refactors, geometry, and final correctness review.
